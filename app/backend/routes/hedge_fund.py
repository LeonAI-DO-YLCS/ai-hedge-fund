...
async def run(
    request_data: HedgeFundRequest, request: Request, db: Session = Depends(get_db)
):
    try:
        ...
        async def event_generator():
            ...
            def progress_handler(agent_name, ticker, status, analysis, timestamp):
                metadata = {}
                try:
                    metadata = json.loads(analysis) if analysis else {}
                except Exception:
                    metadata = {}
            ...
            try:
                ...
                final_data = CompleteEvent(
                    data={
                        ...
                        "current_prices": result.get("data", {}).get(
                            "current_prices", {}
                        ),
                    }
                )
...
async def backtest(
    request_data: BacktestRequest, request: Request, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        async def event_generator():
            ...
            try:
                ...
                backtest_task = asyncio.create_task(
                    backtest_service.run_backtest_async(
        ...
        raise HTTPException(
            status_code=500,
            detail=f"An error occurred while processing the backtest request: {str(e)}",
        )
